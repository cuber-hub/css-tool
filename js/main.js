...
/** Format output with HTML + CSS + Tailwind blocks */
function formatOutput(htmlSnippet, cssSnippet, twSnippet = '') {
  let output = `<!-- HTML -->\n${htmlSnippet}\n\n/* CSS */\n${cssSnippet}`;
  if (twSnippet) {
    output += `\n\n/* Tailwind Classes */\n${twSnippet}`;
  }
...
/** Extract HTML, CSS, and Tailwind from rendered output block */
function extractCodes(outputText = '') {
  const cssMarker = '\n\n/* CSS */\n';
  const twMarker = '\n\n/* Tailwind Classes */\n';
  const htmlPrefix = '<!-- HTML -->\n';
  
  if (!outputText.startsWith(htmlPrefix)) return { html: '', css: '', tw: '' };
  
  let remainder = outputText.slice(htmlPrefix.length);
  let html = '', css = '', tw = '';
  
  if (remainder.includes(twMarker)) {
    const parts = remainder.split(twMarker);
    tw = parts[1].trim();
    remainder = parts[0];
  }
  
  if (remainder.includes(cssMarker)) {
    const parts = remainder.split(cssMarker);
    css = parts[1].trim();
    html = parts[0].trim();
  } else {
    html = remainder.trim();
...
/** Wire Copy HTML, CSS, and Tailwind buttons */
function bindCopyBtn(btnId, codeEl, type) {
  const btn = document.getElementById(btnId);
  if (!btn) return;
  btn.addEventListener('click', () => {
    const codes = extractCodes(codeEl?.textContent || '');
    if (codes[type]) copyToClipboard(codes[type]);
    else if (type === 'tw' && !codes.tw) {
       // if not found in block, maybe it was meant as classes only
       // some tools might not use the block for TW
    }
  });
}

function bindAllCopyButtons(htmlId, cssId, twId, codeEl) {
  bindCopyBtn(htmlId, codeEl, 'html');
  bindCopyBtn(cssId, codeEl, 'css');
  bindCopyBtn(twId, codeEl, 'tw');
}
...
function ensureSharedLayout() {
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-heading">Tools Collection</div>
    <nav class="sidebar-nav">
      ...
      <a href="${base}/pages/button.html" class="nav-item">🔘 Button Tool</a>
      <a href="${base}/pages/buttons-library.html" class="nav-item">🏗️ Buttons Library</a>
  ...
  <header class="topbar">
    <div class="topbar-left">
      ...
      <nav class="topbar-nav">
        <a href="${base}/index.html" class="top-nav-link" id="topNavHome">Home</a>
        <a href="${base}/pages/about.html" class="top-nav-link" id="topNavAbout">About</a>
      </nav>
    </div>
    <div class="topbar-right">
      <button class="theme-toggle-top" id="themeToggleTop" aria-label="Toggle theme">
        <span class="theme-icon">🌙</span>
        <span class="theme-label" style="margin-left:8px; font-size:13px; font-weight:500;">Dark Mode</span>
      </button>
    </div>
  </header>`;
...
const navItems  = document.querySelectorAll('.nav-item, .top-nav-link');
...
// Highlight sidebar/topbar link by current URL on multi-page setup
if (navItems.length) {
  ...
  navItems.forEach(n => {
    ...
    // Exact file matching for permanent highlight
    const hrefSegments = href.split('/');
    const targetFile = hrefSegments[hrefSegments.length - 1];
    
    let isActive = (currentFile === targetFile);
    
    // Fallback for subdirectories or index cases
    if (!isActive) {
        isActive = currentPath.endsWith(href) || 
                   (targetFile === 'index.html' && (currentFile === '' || currentFile === 'index.html'));
    }
...
function applyTheme(dark) {
  ...
  // Update all potential theme toggle elements
  document.querySelectorAll('.theme-icon').forEach(i => i.textContent = icon);
  document.querySelectorAll('.theme-label').forEach(l => l.textContent = label);
  
  // Support legacy single-icon toggles
  const tt = document.getElementById('themeToggleTop');
  if (tt && !tt.querySelector('.theme-icon')) tt.textContent = icon;
...
(function gradientTool() {
  ...
  background: ${bgValue};${blurLine}
    ...
    // Tailwind logic
    const tw = `w-full h-full bg-[${bgValue.replace(/ /g,'_')}]${state.blur > 0 ? ` blur-[${state.blur}px]` : ''}`;
    
    code.textContent = formatOutput('<div class="gradient-box"></div>', css, tw);
  ...
  bindAllCopyButtons('copyGradientHtml', 'copyGradientCss', 'copyGradientTw', code);
...
(function shadowTool() {
  ...
  function render() {
    ...
    // Tailwind logic
    const tw = `shadow-[${value.replace(/ /g,'_')}]`;

    code.textContent = formatOutput(
  ...
  bindAllCopyButtons('copyShadowHtml', 'copyShadowCss', 'copyShadowTw', code);
...
(function borderRadiusTool() {
  ...
  function render() {
    ...
    // Tailwind logic
    const tw = `w-40 h-40 bg-[${state.color}] rounded-[${v.replace(/ /g,'_')}]`;
  ...
  bindAllCopyButtons('copyBorderRadiusHtml', 'copyBorderRadiusCss', 'copyBorderRadiusTw', code);
...
`.container {
  ...
  gap: ${state.gap}px;
    ...
    // Tailwind logic
    const tw = `flex flex-${state.direction === 'row' ? 'row' : (state.direction === 'column' ? 'col' : (state.direction === 'row-reverse' ? 'row-reverse' : 'col-reverse'))} justify-[${state.justify}] items-[${state.align}] flex-${state.wrap} gap-[${state.gap}px]`;

    const items = Array.from({ length: state.count }, (_, i) => `  <div class="item">${i + 1}</div>`).join('\n');
    code.textContent = formatOutput(`<div class="container">
${items}
</div>`, css, tw);
  ...
  bindAllCopyButtons('copyFlexHtml', 'copyFlexCss', 'copyFlexTw', code);
...
`.container {
  ...
  gap: ${state.gap}px;
    ...
    // Tailwind logic
    const tw = `grid grid-cols-[${colDef.replace(/ /g,'_')}] grid-rows-[${rowDef.replace(/ /g,'_')}] gap-[${state.gap}px]`;
...
${items}
  ...
  bindAllCopyButtons('copyGridHtml', 'copyGridCss', 'copyGridTw', code);
...
${keyframes[state.type] || ''}`;
    ...
    // Tailwind logic
    const tw = `animate-[${animName}_${dur}s_${state.easing.replace(/ /g,'_')}_${state.iteration}]`;

    code.textContent = formatOutput('<div class="element"></div>', css, tw);
  ...
  bindAllCopyButtons('copyAnimationHtml', 'copyAnimationCss', 'copyAnimationTw', code);
...
`.button {
  ...
  transition: all 0.2s ease;
    ...
    // Tailwind logic
    const tw = `px-[${state.ph}px] py-[${state.pv}px] bg-[${state.bg}] text-[${state.color}] rounded-[${state.br}px] text-[${state.fs}px] ${state.bw > 0 ? `border-[${state.bw}px]_border-[${state.bc}]` : ''} cursor-pointer transition-all duration-200`;

    code.textContent = formatOutput(`<button class="button">${state.text}</button>`, css, tw);
  ...
  bindAllCopyButtons('copyButtonHtml', 'copyButtonCss', 'copyButtonTw', code);
...
`.glass {
  ...
  border-radius: ${state.radius}px;
    ...
    // Tailwind logic
    const tw = `bg-[${bg.replace(/ /g,'_')}] backdrop-blur-[${state.blur}px] border border-[${border.replace(/ /g,'_')}] rounded-[${state.radius}px]`;

    code.textContent = formatOutput('<div class="glass">Glass Card</div>', css, tw);
  ...
  bindAllCopyButtons('copyGlassHtml', 'copyGlassCss', 'copyGlassTw', code);
...
(function clipTool() {
  ...
  function render() {
    ...
    // Tailwind logic
    const tw = `w-[180px] h-[180px] bg-[${state.color}] clip-path-[${state.clip.replace(/ /g,'_')}]`;
  ...
  bindAllCopyButtons('copyClipHtml', 'copyClipCss', 'copyClipTw', code);
...
@keyframes ripple { 0%{transform:scale(0);opacity:1} 100%{transform:scale(1);opacity:0} }`;
    ...
    const tw = `animate-[spin_${speed}_linear_infinite] border-[${Math.max(3,s/12|0)}px]_solid_rgba(108,99,255,0.2)] border-t-[${state.color}] rounded-full w-[${s}px] h-[${s}px]`;

    code.textContent = formatOutput('<div class="spinner"></div>', cssText, tw);
  ...
  bindAllCopyButtons('copyLoaderHtml', 'copyLoaderCss', 'copyLoaderTw', code);
